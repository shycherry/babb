...
var CoverflowView = Backbone.View.extend({
  ...
  recreateFullDOMCollection: function(){
    ...
    this.collection.forEach(function(item){
      var $div = self.createCellDOM(item, index)      
      index++                 
  ...
  virtualizeDOM : function(){    
    ...
    domCells = this.$divTray.children()
    domCellsCount = domCells.size()
    domCellsFirstIdx = domCells.first().attr('index') - 0
    domCellsLastIdx = domCells.last().attr('index') - 0
    ...
    if(flow<0){
      ...
      for(var i=0; i<nbCellsToAdd; i++){
        ...
        if(indexToAdd >= 0){
          ...
          divToAdd.insertBefore(domCells.first())
        }
      }
    }else{
      //on avance
      ...
      for(var i=0; i<nbCellsToAdd; i++){
        ...
        if(indexToAdd < this.collection.length){
          var itemToAdd = this.collection.at(indexToAdd)
          var divToAdd = this.createCellDOM(itemToAdd, indexToAdd)
          divToAdd.insertAfter(domCells.last())
        }
      }
    }
