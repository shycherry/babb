...
var PlatformsCollectionView = Backbone.View.extend({
	...
	template : _.template( $(BABB.PlatformsConfig.platformsCollectionTemplateId).html() ),
  ...
  initialize : function() {
    ...
    var self = this
    this.el = $(BABB.PlatformsConfig.platformsContainerId)
    _.bindAll(this, 'render')
  ...
  onSelectedChange : function (parPlatform){
    this.el.children('.focus').removeClass('focus')
    if(parPlatform){
      this.temporaryFocusContainer()

      //highlight
      $('#'+parPlatform.id).addClass('focus')

      //change CSS
      var oldCSSPlatform = $('#CSS-Platform')
      oldCSSPlatform.attr('href', '')
      var platformCSSPath = Path.normalize(parPlatform.get('path')+"/style.css")
...
var RomsCollectionView = Backbone.View.extend({
  ...
  selectedRom : null,
  template : _.template( $(BABB.RomsConfig.romsCollectionTemplateId).html() ),
  ...
  initialize : function() {
    ...
    var self = this
    this.el = $(BABB.RomsConfig.romsContainerId)
    _.bindAll(this, 'render')
    _.bindAll(this, 'onSniffed')
    _.bindAll(this, 'onSelectedChange')
    _.bindAll(this, 'temporaryFocusContainer')
...
platformsCollectionView.validCallback = function(parPlatform){
  if( ! parPlatform.isAvailable()){
    BABB.EventEmitter.trigger('error', parPlatform+' is not available')
    return
  }
